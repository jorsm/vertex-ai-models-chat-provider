...
- `updateLabels()`: When the user label is enabled, fetches and caches the current identity from `AuthManager` and propagates it as a provider baseline. The `gcloud config get-value account` fallback accepts only the first output line and only when it is an email address, so CLI notices cannot become a label. Actual labels are resolved per request. Values are sanitized to GCP label requirements (lowercase alphanumeric, hyphens, and underscores; maximum 63 characters; a leading letter is enforced with `v_`).
